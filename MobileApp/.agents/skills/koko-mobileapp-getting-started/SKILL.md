...
1. **Find the git repo root once.** Run `git rev-parse --show-toplevel` — it's the folder that contains
...
2. **Activate the central getting-started checklist.**
   Read and follow the blueprint at `../skills/getting-started/SKILL.md` (repo-root `skills/getting-started/`).

3. **Initialize the Phase 1 progress tracker — at the repo root, NEVER inside `MobileApp/`.**
...
4. **Is the product defined yet?** Before building features, `AiGuidelines/project/prd.md` / `user_flow.md`
...
5. **Drive Phase 1 Implementation:**

---
name: kmp-skills
description: Index and router for ALL KMPStarterKit developer skills — running the app, adding screens/models/API calls/preferences/permissions, Firebase, auth, web-proxy, publishing, subscriptions, credits, ads, analytics, notifications, onboarding, growth. Consult this FIRST for any KMPStarterKit task; it maps the request to the right skill in the `skills/` folder and tells you how to load it.
---

# KMPStarterKit — Skills Index & Router

The full skill library lives once, canonically, in the **`skills/` directory at the repository root**
(the same directory as this `.agents/` folder). This index maps a task to the right skill so the agent
can load only what it needs.

## How to use a skill

1. Find the task in the index below.
2. **Read the matching file** `skills/<name>/SKILL.md` (relative to the repository root; from *this*
   file that is `../../../skills/<name>/SKILL.md`).
...
> Names below match the folder names under `skills/`.
...
*One source of truth: edit the real skills under `skills/`. This index only points to them.*

...
- **Unit & Compose UI tests** — `kotlinx-coroutines-test` for `Flow` / state-holder tests; `runComposeUiTest` for headless UI tests on JVM. Run via `./gradlew :shared:jvmTest :shared:testAndroidHostTest`.
